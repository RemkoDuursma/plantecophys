...
- Did you measure dark respiration and use that in the fit, or was it estimated from the A-C~i~ curve?
...
f <- fitaci(mydata, gmeso=0.2)
```

In this case the equations from [Ethier and Livingston (2004)](http://onlinelibrary.wiley.com/doi/10.1111/j.1365-3040.2004.01140.x/full) are used.

- Calculate the chloroplastic CO~2~ concentration, and fit normally, like this:

```{r}
# Assume a mesophyll conductance of 0.2 mol m-2 s-1 bar-1
mydata$Cc <- with(mydata, Ci - Photo/0.2)

# Fit normally, but make sure to use Cc!
f <- fitaci(mydata, varnames=list(ALEAF="Photo", Ci="Cc", Tleaf="Tleaf"))
```
